...
function requireAuthentication() {
  ...
  const destination = `chess/room/index.html${window.location.search}`;
...
function normalizePiece(piece) {
  ...
  const [color, type] = piece.toLowerCase().split("_");
...
function syncBoardFromState(state) {
  ...
  const playerIsInCheck = Boolean(state.inCheck) && state.currentTurn === playerColor && !gameFinished;
  if (hasSyncedInitialState && playerIsInCheck && !playerWasInCheck) {
    window.showToast?.("Você está em xeque.", "warning");
  }
  playerWasInCheck = playerIsInCheck;
...
function showResultOverlay() {
  ...
  const isDraw = finishReason?.startsWith("DRAW_") || finishReason === "STALEMATE";
  ...
  const reasonMessages = {
    ...
    DRAW_FIFTY_MOVE_RULE: "Empate pela regra dos 50 lances.",
...
async function submitRematchAction(action) {
  ...
  try {
    const response = await apiRequest(
      `/chess/rooms/${encodeURIComponent(roomCode)}/rematch/${action}`,
      { method: "POST" },
    );
...
async function submitMatchAction(path) {
  ...
  renderDrawOffer();
  try {
    const state = await apiRequest(`/chess/rooms/${encodeURIComponent(roomCode)}/state/${path}`, {
      method: "POST",
    });
...
async function requestTimeoutConfirmation() {
  ...
  try {
    const state = await apiRequest(`/chess/rooms/${encodeURIComponent(roomCode)}/state/timeout`, {
      method: "POST",
    });
    syncBoardFromState(state);
  } catch (error) {
...
function updateTurnLabel(message) {
  ...
  if (roomStatus === "FINISHED" || gameFinished) {
    if (finishReason?.startsWith("DRAW_") || finishReason === "STALEMATE") {
    ...
    const playerWon = winnerColor === playerColor;
    const reasonLabel = finishReason === "TIMEOUT" ? "por tempo"
      : finishReason === "CHECKMATE" ? "por xeque-mate" : "";
...
async function loadRoom() {
  ...
  try {
    const path = mode === "private"
      ? `/chess/rooms/${encodeURIComponent(roomCode)}/join`
      : `/chess/rooms/${encodeURIComponent(roomCode)}`;
...
async function refreshRoom() {
  ...
  try {
    const room = await apiRequest(`/chess/rooms/${encodeURIComponent(roomCode)}`);
    applyRoom(room);
    await loadGameState();
  } catch (error) {
...
function setupBoard() {
  ...
  backRank.forEach((type, column) => {
    board[0][column] = `black ${type}`;
    board[1][column] = "black pawn";
    board[6][column] = "white pawn";
    board[7][column] = `white ${type}`;
  });
}
...
async function showMoves(row, column) {
  ...
  try {
    const response = await apiRequest(
      `/chess/rooms/${encodeURIComponent(roomCode)}/state/legal-moves?row=${row}&column=${column}`,
    );
    ...
    selectedLegalMoves = response.moves || [];
    selectedLegalMoves.forEach((move) => {
      boardElement
...
async function submitMove(from, to, promotion = null) {
  ...
  try {
    const state = await apiRequest(`/chess/rooms/${encodeURIComponent(roomCode)}/state/moves`, {
      method: "POST",
      body: JSON.stringify({
        fromRow: from.row,
...
function canSelectPiece(row, column) {
  return isOwnPiece(board[row][column]) && currentTurn === playerColor;
}
...
function movePiece(targetRow, targetColumn) {
  ...
  selectedCell = null;
  const legalMove = selectedLegalMoves.find((move) => move.toRow === targetRow && move.toColumn === targetColumn);
  if (legalMove?.promotionRequired) {
    pendingPromotionMove = { from, to: { row: targetRow, column: targetColumn } };
    window.showToast?.("Escolha uma peça para promoção.", "warning");
    document.querySelector("#promotion-overlay").hidden = false;
    return;
  }
  ...
  for (let displayRow = 0; displayRow < 8; displayRow += 1) {
    for (let displayColumn = 0; displayColumn < 8; displayColumn += 1) {
      ...
      if (board[row][column]) {
        ...
        piece.className = `chess-piece ${board[row][column]}`;
        piece.textContent = chessPieceSymbol(board[row][column]);
...
function chessPieceSymbol(piece) {
  const symbols = {
...
document.querySelectorAll("[data-promotion]").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    document.querySelector("#promotion-overlay").hidden = true;
    submitMove(move.from, move.to, button.dataset.promotion);
  });
});
