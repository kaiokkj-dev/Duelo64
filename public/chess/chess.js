...
async function syncMatchmakingStatus() {
  ...
  try {
    applyMatchmakingStatus(await apiRequest("/matchmaking/chess/queue/status"));
  } catch (error) {
...
quickPlayButton?.addEventListener("click", async () => {
  const destination = `chess/index.html?action=quick-play&time=${selectedTime}`;
  ...
  try {
    const response = matchmakingStatus === "QUEUED"
      ? await apiRequest("/matchmaking/chess/queue", { method: "DELETE" })
      : await apiRequest("/matchmaking/chess/queue", {
        method: "POST",
...
createPrivateRoomButton?.addEventListener("click", async () => {
  if (!isAuthenticated()) {
    requestAuthentication("chess/index.html?action=create-private");
    return;
  }
  ...
  try {
    const room = await apiRequest("/chess/rooms", {
      method: "POST",
      body: JSON.stringify({ timeControlMinutes: selectedTime }),
    });
    ...
    if (error.status === 401 || error.status === 403) {
      handleUnauthorized("chess/index.html?action=create-private");
      return;
    }
...
joinForm?.addEventListener("submit", async (event) => {
  ...
  const destination = `chess/room/index.html?mode=private&code=${encodeURIComponent(code)}&time=10`;
  ...
  try {
    const room = await apiRequest(`/chess/rooms/${encodeURIComponent(code)}/join`, {
      method: "POST",
    });
